class Product:
    ...
    def create_product(data):
        ...
        INSERT INTO products (name, description, price, sale_price, sku, stock_quantity, 
                            category_id, images, status, weight, dimensions, is_featured, 
                            tags, meta_title, meta_description, meta_keywords, created_at)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        params = (
            data['name'], data['description'], data['price'], data.get('sale_price'),
            data['sku'], data['stock_quantity'], data.get('category_id'), 
...
class Product:
    ...
    def create_product(data):
        ...
        params = (
            ...
            data.get('weight'), data.get('dimensions'), data.get('is_featured', False),
            json.dumps(data.get('tags', [])), data.get('meta_title', ''),
            data.get('meta_description', ''), data.get('meta_keywords', ''), datetime.now()
        )
